...
class SislWFSXWrapper(SislWrapper):
    ...
    def __init__(self, geom, ham, wfsx_sile, spin, ispin=None, shift_fermi=None):
    ...
    def read_all(self):
        ...
        for wfc in self.wfsx_sile.yield_eigenstate():
            Overlaps.append(self.ham.Sk(wfc.info['k'], format='dense').T)
            #Smh = sqrtm(self.ham.Sk(wfc.info['k'], format='dense'))
        ...
        self.overlaps = Overlaps[~is_duplicate]
    ...
    def write_to_netcdf(self, fname):
        ...
        root.createVariable("overlaps_real", datatype=float,
                            dimensions=("nkpt", "nbasis", "nbasis"))
        root.createVariable("overlaps_imag", datatype=float,
                            dimensions=("nkpt", "nbasis", "nbasis"))
        ...
        root.variables["overlaps_real"][:] = self.overlaps.real
        root.variables["overlaps_imag"][:] = self.overlaps.imag
...
def write_to_netcdf(folder=None, spin="soc", fdf_fname="siesta.fdf",
    ...
    model = SislWFSXWrapper(geom, ham, wfsx_sile, spin=Spin(spin),
                            ispin=None, shift_fermi=-efermi)
...
write_to_netcdf(folder='../siesta', fdf_fname='in.fdf',
                wfxfname="../siesta/G_hBN.selected.WFSX",
                kwfname='kpoints',
                ncfname='wfk.nc')
